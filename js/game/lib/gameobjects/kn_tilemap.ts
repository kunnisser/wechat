...
class TileMap {
    ...
    for (let md of this.mapData) {
      md.data.forEach((m, index) => {
        if (m > 0) {
          this.tile.addFrame(
            textures[m - 1],
            this.tileWidth * (index % this.size_x),
            this.tileHeight * ~~(index / this.size_x)
          );
        }
      });
    }
  ...
  public generateMulteMapData() {
    this.mapData = Array.from(
      { length: this.size_x * this.size_y },
      (t, v) => 1
    );
    ...
    for (let i = 1, l = this.mapData.length; i <= l; i++) {
      ...
      if (
        ~~(Math.random() * 100) < 40 ||
        this.isMapBoundary(this.multeMapData, tmpArr)
      ) {
    ...
    this.multeMapData.forEach((mdArr, col) => {
      mdArr.forEach((md, row) => {
        ...
        if (
          ...
          const outwall =
            this.checkIntervalWalls([col, row]) -
            this.checkNeighborWalls([col, row]);
          if (md === this.mapWall) {
            wall >= 4
              ? (this.multeMapData[col][row] = this.mapWall)
              : (this.multeMapData[col][row] = 1);
          } else {
            wall >= 5
              ? (this.multeMapData[col][row] = this.mapWall)
              : (this.multeMapData[col][row] = 1);
  ...
  public checkNeighborWalls(point: Array<number>) {
    ...
    for (let y = -1; y <= 1; y++) {
      for (let x = -1; x <= 1; x++) {
        const checkMd =
          this.multeMapData[point[0] + y] &&
          this.multeMapData[point[0] + y][point[1] + x];
        if (x !== 0 || y !== 0) {
