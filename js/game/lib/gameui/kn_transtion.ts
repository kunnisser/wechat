...
class KnTransition {
  ...
  renderMask(size: number) {
    this.mask.clear();
    this.mask.generateRect(
      0x000000,
      [this.game.config.half_w, this.game.config.half_h, size, size],
      !0
    );
  }

  generateOverlay() {
    const blackRect = this.game.add
      .graphics()
      .generateRect(0x000000, [0, 0, 1, 1]);
    this.overlay = TransformImage.transformToSprite(
      this.game,
      blackRect,
      this.game.world
    );
