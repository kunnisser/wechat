 ...
 * @LastEditTime: 2024-02-11 00:48:30
 * @FilePath: /wechat/js/game/src/card/checkerboard/checkerCard.ts
 * @Description: ---- 卡牌外壳 ----
 */
...
class CheckerCardWrap extends KnGroup {
  ...
  cardMoveLogic(targetCard: CheckerCardWrap, followCard: CheckerCardWrap) {
    ...
    moveCardTween(
      ...
      {
        x:
          targetIndices[0] * (this.wrap.width + 2) +
          this.game.config.half_w,
        y:
          targetIndices[1] * this.wrap.height +
          this.game.config.half_h,
      },
      () => {
        // 被处理的卡牌重置
        targetCard.position.set(
          followIndices[0] * (this.wrap.width + 2) +
            this.game.config.half_w,
          followIndices[1] * this.wrap.height + this.game.config.half_h
        );
    ...
    moveCardTween(this.parent.tween, followCard, {
      x:
        currentIndices[0] * (this.wrap.width + 2) +
        this.game.config.half_w,
      y:
        currentIndices[1] * this.wrap.height + this.game.config.half_h,
    });
